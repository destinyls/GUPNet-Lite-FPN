...
DATASETS._module_dict.pop('NuScenesMonoDataset')
...
class NuScenesMonoDataset(CocoDataset):
    ...
    def format_output(self, results):
        ...
        img_metas = results["img_metas"][0].data if isinstance(results["img_metas"], list) else results["img_metas"].data
        info = results["info"][0] if isinstance(results["info"], list) else results["info"]
        img = results["img"][0].data if isinstance(results["img"], list) else results["img"].data
        coord_range = info["coord_range"]
        cam2img = np.array(img_metas["cam2img"])
        P = np.hstack((cam2img, np.zeros((cam2img.shape[0], 1), dtype=np.float32))).astype(np.float32)
        return img, P, coord_range, targets, info
