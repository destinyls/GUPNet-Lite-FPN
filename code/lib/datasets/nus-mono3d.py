...
train_pipeline = [
    ...
    dict(type='GUPNetPreprocess', class_names=class_names, split='train'),
    dict(
        type='Collect3D',
        keys=[
            'img', 'gt_bboxes', 'gt_labels', 'attr_labels', 'gt_bboxes_3d',
            'gt_labels_3d', 'centers2d', 'depths', 'info', 'targets'
        ]),
]
test_pipeline = [
    dict(type='LoadImageFromFileMono3D'),
    dict(
        ...
        transforms=[
            ...
            dict(
                ...
                with_label=False),
            dict(type='GUPNetPreprocess', class_names=class_names, split='test'),
            dict(type='Collect3D', keys=['img', 'info', 'targets']),
        ])
]
