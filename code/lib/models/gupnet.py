...
from lib.backbones.resnet import resnet18, resnet34
...
class GUPNet(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None):
        ...
        self.feat_up = globals()[neck](channels[self.first_level:], scales_list=scales) if "dla" in backbone else None
    ...
    def forward(self, input, coord_ranges,calibs, targets=None, K=50, mode='train'):
        ...
        '''
        ret = {}
        ret['heatmap']=self.heatmap(feats[0])
        ret['offset_2d']=self.offset_2d(feats[0])
        ret['size_2d']=self.size_2d(feats[0])
    ...
    def get_roi_feat_by_mask(self,feats,box2d_maps,inds,mask,calibs,coord_ranges,cls_ids):
        ...
        if num_masked_bin!=0:
            ...
            size3d_offset = size3d_offset[:,:,0,0]
