...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.in_planes = self.out_channel
        self.deconv_with_bias = False
        self.deconv_layers_1 = self._make_deconv_layer(self.out_channel//2, 4)
        self.deconv_layers_2 = self._make_deconv_layer(self.out_channel//4, 4)
        self.deconv_layers_3 = self._make_deconv_layer(self.out_channel//8, 4)
        self.init_deconv_weights()
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1).contiguous().view(B, -1, H//32, W//32)
    ...
    def init_deconv_weights(self, pretrained=True):
        if pretrained:
...
def Swin_T(config):
    ...
    state_dict = load_state_dict_from_url(model_urls['swin_tiny_patch4_window7_224'], progress=True, map_location='cpu')
...
def Swin_S(config):
    ...
    state_dict = load_state_dict_from_url(model_urls['swin_small_patch4_window7_224'], progress=True, map_location='cpu')
    model.load_state_dict(state_dict, strict=False)
    return model
...
def Swin_B(config):
    ...
    state_dict = load_state_dict_from_url(model_urls['swin_base_patch4_window7_224_in22k'], progress=True, map_location='cpu')
    model.load_state_dict(state_dict, strict=False)
    return model

def Swin_L(config):
    embed_dim, depths, num_heads, window_size = 192, [2, 2, 18, 2], [6, 12, 24, 48], 6
    model = SwinTransformer(
        img_size=config.DATA.IMG_SIZE,
        patch_size=config.MODEL.SWIN.PATCH_SIZE,
        in_chans=config.MODEL.SWIN.IN_CHANS,
    ...
    state_dict = load_state_dict_from_url(model_urls['swin_large_patch4_window7_224_in22k'], progress=True, map_location='cpu')
    model.load_state_dict(state_dict, strict=False)
    return model
