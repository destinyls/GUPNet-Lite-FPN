...
class KittiConverter:
    ...
    def nuscenes_gt_to_kitti(self) -> None:
        ...
        label_folder = os.path.join(self.nusc_kitti_dir, "training", 'label_2')
        calib_folder = os.path.join(self.nusc_kitti_dir, "training", 'calib')
        image_folder = os.path.join(self.nusc_kitti_dir, "training", 'image_2')
        lidar_folder = os.path.join(self.nusc_kitti_dir, "training", 'velodyne')
        imageset_folder = os.path.join(self.nusc_kitti_dir, "ImageSets")
        ...
        token_idx = 0 if 'train' in self.split else 200000
        tokens = []
        for sample_token in sample_tokens:
            ...
            sample_token_name = sample_token + "_" + self.cam_name
            src_im_path = os.path.join(self.nusc.dataroot, filename_cam_full)
            dst_im_path = os.path.join(image_folder, token + '.jpg')
            if not os.path.exists(dst_im_path):
                '''
                im = Image.open(src_im_path)
                im.save(dst_im_path, "PNG")
                '''
                cmd = 'cp ' + src_im_path + ' ' + dst_im_path
                os.system(cmd)            
        ...
        imageset = 'train.txt' if "train" in self.split else 'val.txt'
        with open(os.path.join(imageset_folder, imageset),'w') as f:
            for token in tokens:
                f.write(token)
                f.write("\n")
    ...
    def _box_to_sample_result(self, sample_token: str, box: Box, attribute_name: str = '') -> Dict[str, Any]:
        ...
        sample_result['detection_name'] = detection_name.lower()
